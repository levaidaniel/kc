...
if printf "passwd -A ssh-rsa,PuBkEy_+*()[]{}';,.!@#^&=-/|_comment-rsa:2048 -P bcrypt\n${PASSWORD}\n${PASSWORD}\n" |${KC_RUN} -b -k ${KC_DB} -P bcrypt -p ${KC_PASSFILE};then
	echo "$0 test ok (SSH agent, passwd bcrypt #2)!"
else
	echo "$0 test failed (SSH agent, passwd bcrypt #2)!"
	exit 1
fi

# Also fake an invalid parameter
if printf "passwd --help\npasswd -P sha512\n${PASSWORD}\n${PASSWORD}\n" |${KC_RUN} -b -k ${KC_DB} -P bcrypt -A "ssh-rsa,PuBkEy_+*()[]{}';,.!@#^&=-/|_comment-rsa:2048";then
