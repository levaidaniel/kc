...
void cmd_getnum(const int, const int);
...
xmlNodePtr	find_keychain(const xmlChar *, char);
xmlNodePtr	find_key(const int);
void		_rl_push_buffer(void);
char		*get_random_str(const unsigned int, const unsigned char);
xmlChar		*parse_randoms(const xmlChar *);
xmlChar		*get_line(const xmlChar *, const unsigned int);
